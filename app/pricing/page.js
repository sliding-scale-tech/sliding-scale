import Navbar from '../components/Navbar'
import WfPageInit from '../components/WfPageInit'
import CtaFooter from '../components/CtaFooter'

const PAGE_ID = '6a07fc541dbdc952efdebd32'
const SITE_ID = '69eb3a8ab30f30e3f2767ea3'

const blurSlideStyle = {
  filter: 'blur(5px)',
  opacity: 0,
  WebkitTransform: 'translate3d(0, 2rem, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0)',
  MozTransform: 'translate3d(0, 2rem, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0)',
  msTransform: 'translate3d(0, 2rem, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0)',
  transform: 'translate3d(0, 2rem, 0) scale3d(1, 1, 1) rotateX(0) rotateY(0) rotateZ(0) skew(0, 0)',
}

function CheckIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M0 12C0 5.37258 5.37258 0 12 0C18.6274 0 24 5.37258 24 12C24 18.6274 18.6274 24 12 24C5.37258 24 0 18.6274 0 12Z" fill="#D1FADF"></path>
      <path fillRule="evenodd" clipRule="evenodd" d="M17.096 7.39004L9.93602 14.3L8.03602 12.27C7.68602 11.94 7.13602 11.92 6.73602 12.2C6.34602 12.49 6.23602 13 6.47602 13.41L8.72602 17.07C8.94602 17.41 9.32601 17.62 9.75601 17.62C10.166 17.62 10.556 17.41 10.776 17.07C11.136 16.6 18.006 8.41004 18.006 8.41004C18.906 7.49004 17.816 6.68004 17.096 7.38004V7.39004Z" fill="#12B76A"></path>
    </svg>
  )
}

function ArrowButtonSvg() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 16 16" fill="none" preserveAspectRatio="xMidYMid meet" aria-hidden="true" role="img">
      <g clipPath="url(#clip0_6234_2207_pricing)">
        <path d="M3.33301 8H12.6663" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round"></path>
        <path d="M10 10.6667L12.6667 8" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round"></path>
        <path d="M10 5.33333L12.6667 8" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round"></path>
      </g>
      <defs>
        <clipPath id="clip0_6234_2207_pricing">
          <rect width="16" height="16" fill="currentColor"></rect>
        </clipPath>
      </defs>
    </svg>
  )
}

function PhoneSvg() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 32 32" fill="none" preserveAspectRatio="xMidYMid meet" aria-hidden="true" role="img">
      <path d="M20.7415 19.3969L20.1343 20.0363C20.1343 20.0363 18.6908 21.556 14.7508 17.4079C10.8108 13.2598 12.2543 11.7401 12.2543 11.7401L12.6367 11.3375C13.5788 10.3457 13.6676 8.75329 12.8457 7.5908L11.1643 5.21282C10.147 3.774 8.18128 3.58393 7.01527 4.81152L4.92247 7.01485C4.34431 7.62354 3.95687 8.4126 4.00385 9.28792C4.12405 11.5273 5.08095 16.3454 10.4205 21.9669C16.0829 27.9283 21.3958 28.1652 23.5684 27.9508C24.2556 27.883 24.8532 27.5124 25.3348 27.0053L27.2289 25.0112C28.5074 23.6652 28.1469 21.3576 26.5111 20.416L23.9638 18.9498C22.8897 18.3315 21.581 18.5131 20.7415 19.3969Z" fill="currentColor"></path>
      <path d="M17.6793 2.50677C17.7676 1.96158 18.283 1.59175 18.8282 1.68001C18.8619 1.68647 18.9705 1.70677 19.0274 1.71944C19.1411 1.74477 19.2998 1.78378 19.4977 1.84142C19.8934 1.95668 20.4462 2.14655 21.1097 2.45073C22.4381 3.05973 24.2058 4.12508 26.0402 5.95948C27.8746 7.79387 28.94 9.56163 29.549 10.89C29.8532 11.5535 30.043 12.1063 30.1583 12.502C30.2159 12.6999 30.2549 12.8586 30.2803 12.9723C30.293 13.0292 30.3022 13.0749 30.3087 13.1087L30.3163 13.1503C30.4046 13.6954 30.0381 14.2321 29.493 14.3204C28.9493 14.4084 28.4372 14.0404 28.3468 13.4977C28.344 13.4832 28.3363 13.444 28.3281 13.4071C28.3117 13.3333 28.2832 13.2162 28.2381 13.0613C28.1479 12.7515 27.9912 12.2911 27.7309 11.7235C27.2112 10.5898 26.2765 9.02417 24.626 7.37369C22.9755 5.72321 21.41 4.78855 20.2762 4.26877C19.7087 4.00856 19.2482 3.85185 18.9384 3.76162C18.7836 3.71651 18.5889 3.67183 18.515 3.65539C17.9724 3.56496 17.5913 3.0504 17.6793 2.50677Z" fill="currentColor"></path>
      <path fillRule="evenodd" clipRule="evenodd" d="M17.981 7.10605C18.1327 6.57502 18.6862 6.26753 19.2172 6.41925L18.9425 7.38078C19.2172 6.41925 19.2172 6.41925 19.2172 6.41925L19.2192 6.4198L19.2212 6.42039L19.2256 6.42169L19.236 6.42478L19.2624 6.43299C19.2825 6.43942 19.3076 6.4478 19.3375 6.45842C19.3974 6.47967 19.4765 6.50989 19.5736 6.55149C19.7679 6.63474 20.0333 6.7633 20.3593 6.95625C21.0119 7.34249 21.9028 7.98406 22.9494 9.03069C23.9961 10.0773 24.6376 10.9682 25.0239 11.6209C25.2168 11.9469 25.3454 12.2123 25.4286 12.4065C25.4703 12.5036 25.5005 12.5828 25.5217 12.6426C25.5323 12.6726 25.5407 12.6977 25.5471 12.7178L25.5554 12.7442L25.5584 12.7545L25.5597 12.7589L25.5603 12.761C25.5603 12.761 25.5609 12.7629 24.5994 13.0376L25.5609 12.7629C25.7126 13.2939 25.4051 13.8474 24.8741 13.9992C24.3476 14.1496 23.799 13.8486 23.6418 13.3258L23.6369 13.3115C23.6298 13.2914 23.615 13.252 23.5904 13.1944C23.541 13.0793 23.4516 12.891 23.3027 12.6395C23.0054 12.1372 22.4685 11.3782 21.5352 10.4449C20.602 9.51163 19.843 8.97469 19.3406 8.6774C19.0892 8.52858 18.9009 8.43912 18.7858 8.38978C18.7282 8.36509 18.6887 8.35038 18.6687 8.34327L18.6543 8.33835C18.1316 8.18118 17.8306 7.63258 17.981 7.10605Z" fill="currentColor"></path>
    </svg>
  )
}

export default function PricingPage() {
  return (
    <>
      <WfPageInit pageId={PAGE_ID} siteId={SITE_ID} />
      <div className="div-block-66">
        <Navbar currentPage="pricing" />
        <div className="main-wrapper">
          <section className="uui-section_pricing01">
            <div className="uui-page-padding-2">
              <div className="uui-container-large-2">
                <div className="uui-padding-vertical-xhuge-2 sep-pricing">
                  <div
                    data-w-id="5b26d1ee-b4da-3aef-ae8b-03878cfdc0ad"
                    style={blurSlideStyle}
                    className="heading-layout"
                  >
                    <div className="heading-wrap">
                      <h2 className="heading-style-h2">Simple, Transparent &amp; Affordable</h2>
                    </div>
                  </div>
                  <div className="w-layout-grid uui-pricing01_component">
                    {/* Left column: feature list */}
                    <div className="uui-pricing01_content">
                      <div className="w-layout-grid uui-pricing01_content-list">
                        <div id="w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc0b4-efdebd32" className="uui-pricing01_content-item">
                          <div className="uui-pricing01_item-icon-wrapper">
                            <div className="uui-icon-featured-outline-large">
                              <div className="uui-icon-1x1-xsmall w-embed">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M6.09436 11.2288C6.03221 10.8282 5.99996 10.4179 5.99996 10C5.99996 5.58172 9.60525 2 14.0526 2C18.4999 2 22.1052 5.58172 22.1052 10C22.1052 10.9981 21.9213 11.9535 21.5852 12.8345C21.5154 13.0175 21.4804 13.109 21.4646 13.1804C21.4489 13.2512 21.4428 13.301 21.4411 13.3735C21.4394 13.4466 21.4493 13.5272 21.4692 13.6883L21.8717 16.9585C21.9153 17.3125 21.9371 17.4895 21.8782 17.6182C21.8266 17.731 21.735 17.8205 21.6211 17.8695C21.4911 17.9254 21.3146 17.8995 20.9617 17.8478L17.7765 17.3809C17.6101 17.3565 17.527 17.3443 17.4512 17.3448C17.3763 17.3452 17.3245 17.3507 17.2511 17.3661C17.177 17.3817 17.0823 17.4172 16.893 17.4881C16.0097 17.819 15.0524 18 14.0526 18C13.6344 18 13.2237 17.9683 12.8227 17.9073M7.63158 22C10.5965 22 13 19.5376 13 16.5C13 13.4624 10.5965 11 7.63158 11C4.66668 11 2.26316 13.4624 2.26316 16.5C2.26316 17.1106 2.36028 17.6979 2.53955 18.2467C2.61533 18.4787 2.65322 18.5947 2.66566 18.6739C2.67864 18.7567 2.68091 18.8031 2.67608 18.8867C2.67145 18.9668 2.65141 19.0573 2.61134 19.2383L2 22L4.9948 21.591C5.15827 21.5687 5.24 21.5575 5.31137 21.558C5.38652 21.5585 5.42641 21.5626 5.50011 21.5773C5.5701 21.5912 5.67416 21.6279 5.88227 21.7014C6.43059 21.8949 7.01911 22 7.63158 22Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path>
                                </svg>
                              </div>
                            </div>
                          </div>
                          <div className="uui-pricing01_item-text-wrapper">
                            <div className="div-block-58">
                              <h5 className="uui-heading-xxsmall-2">Clarity Engine</h5>
                              <div className="uui-badge is-primary">
                                <div className="text-block-45">Complimentary</div>
                              </div>
                            </div>
                            <div className="uui-space-xxsmall-2"></div>
                            <div className="uui-text-size-medium-2">1-on-1 Product Discovery with a global product builder</div>
                          </div>
                        </div>
                        <div id="w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc0c2-efdebd32" className="uui-pricing01_content-item">
                          <div className="uui-pricing01_item-icon-wrapper">
                            <div className="uui-icon-featured-outline-large">
                              <div className="uui-icon-1x1-xsmall w-embed">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M13.0001 2L4.09356 12.6879C3.74475 13.1064 3.57035 13.3157 3.56768 13.4925C3.56537 13.6461 3.63384 13.7923 3.75336 13.8889C3.89085 14 4.16328 14 4.70814 14H12.0001L11.0001 22L19.9067 11.3121C20.2555 10.8936 20.4299 10.6843 20.4325 10.5075C20.4348 10.3539 20.3664 10.2077 20.2468 10.1111C20.1094 10 19.8369 10 19.2921 10H12.0001L13.0001 2Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path>
                                </svg>
                              </div>
                            </div>
                          </div>
                          <div className="uui-pricing01_item-text-wrapper">
                            <div className="div-block-58">
                              <h5 className="uui-heading-xxsmall-2">Conversion-First UX</h5>
                              <div className="uui-badge is-primary">
                                <div className="text-block-46">Complimentary</div>
                              </div>
                            </div>
                            <div className="uui-space-xxsmall-2"></div>
                            <div className="uui-text-size-medium-2">High-performance design built for real users</div>
                          </div>
                        </div>
                        <div id="w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc0d0-efdebd32" className="uui-pricing01_content-item">
                          <div className="uui-pricing01_item-icon-wrapper">
                            <div className="uui-icon-featured-outline-large">
                              <div className="uui-icon-1x1-xsmall w-embed">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M11 3H7.8C6.11984 3 5.27976 3 4.63803 3.32698C4.07354 3.6146 3.6146 4.07354 3.32698 4.63803C3 5.27976 3 6.11984 3 7.8V16.2C3 17.8802 3 18.7202 3.32698 19.362C3.6146 19.9265 4.07354 20.3854 4.63803 20.673C5.27976 21 6.11984 21 7.8 21H16.2C17.8802 21 18.7202 21 19.362 20.673C19.9265 20.3854 20.3854 19.9265 20.673 19.362C21 18.7202 21 17.8802 21 16.2V13M12 8H16V12M15.5 3.5V2M19.4393 4.56066L20.5 3.5M20.5103 8.5H22.0103M3 13.3471C3.65194 13.4478 4.31987 13.5 5 13.5C9.38636 13.5 13.2653 11.3276 15.6197 8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path>
                                </svg>
                              </div>
                            </div>
                          </div>
                          <div className="uui-pricing01_item-text-wrapper">
                            <h5 className="uui-heading-xxsmall-2">Bulletproof QA</h5>
                            <div className="uui-space-xxsmall-2"></div>
                            <div className="uui-text-size-medium-2">Pre-launch testing, edge-case handling, and stablization</div>
                          </div>
                        </div>
                      </div>
                    </div>

                    {/* Right column: Rocket Launch MVP plan */}
                    <div id="w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc0da-efdebd32" className="uui-pricing01_plan">
                      <div className="uui-pricing01_plan-content">
                        <div className="uui-pricing01_content-top">
                          <div className="uui-pricing01_top-left">
                            <div className="uui-pricing01_plan-name-wrapper">
                              <h2 className="uui-heading-xsmall">Rocket Launch MVP</h2>
                            </div>
                            <div className="uui-pricing01_top-right-price">$10,000</div>
                            <div className="uui-text-size-medium-2 subtitle">A structured, high-velocity execution model designed to take your product from idea to launch in <strong>4 weeks</strong></div>
                          </div>
                          <div className="uui-pricing01_top-right">
                            <div className="modal-cta-icon-frame">
                              <img src="/images/start-up_18586692.webp" loading="lazy" width="40" alt="" />
                            </div>
                          </div>
                        </div>
                        <div className="uui-pricing01_divider"></div>
                        <div className="uui-pricing01_feature-list">
                          {[
                            { id: 'w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc0ec-efdebd32', text: 'Launch your app in 4 weeks' },
                            { id: 'w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc0f1-efdebd32', text: 'Scalable Core (full- stack development)' },
                            { id: 'w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc0f6-efdebd32', text: 'Launch Accelerator' },
                            { id: 'w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc0fb-efdebd32', text: 'Daily async updates' },
                            { id: 'w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc100-efdebd32', text: 'Weekly demo syncs' },
                            { id: 'w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc105-efdebd32', text: 'Go-live support' },
                          ].map((item) => (
                            <div key={item.id} id={item.id} className="uui-pricing01_feature">
                              <div className="uui-pricing01_icon-wrapper">
                                <div className="uui-icon-1x1-xsmall w-embed"><CheckIcon /></div>
                              </div>
                              <div className="uui-text-size-medium-2">{item.text}</div>
                            </div>
                          ))}
                        </div>
                        <div className="uui-pricing01_divider"></div>
                      </div>
                      <div className="dots-partern"></div>
                      <a href="https://calendar.app.google/PrdZAdEpBpV5YHCKA" target="_blank" className="button-2 w-inline-block">
                        <div className="button-inner">
                          <div className="button-contact-avt">
                            <img loading="lazy" src="/images/contact-avt.png" alt="avatar" className="button-contact-avt-img" />
                            <img loading="lazy" src="/images/contact-avt-1.png" alt="avatar" className="button-contact-avt-img" />
                          </div>
                          <div className="button-text-wrap">
                            <div className="button-text">Get Package</div>
                          </div>
                          <div className="button-icon">
                            <div className="icon-button w-embed"><ArrowButtonSvg /></div>
                          </div>
                        </div>
                        <div className="button-innershadow"></div>
                      </a>
                    </div>
                  </div>
                </div>
              </div>

              {/* Momentum Support */}
              <div className="padding-global mmntm-box">
                <div className="container-base">
                  <div className="capabilities-layout momentm-sprt-pck">
                    <div
                      data-w-id="5b26d1ee-b4da-3aef-ae8b-03878cfdc11a"
                      style={blurSlideStyle}
                      className="capabilities-bottom"
                    >
                      <div className="capabilities-grid">
                        <div id="w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc11c-efdebd32" className="modal-cta capabilities pricing-box">
                          <div className="div-block-60">
                            <div className="modal-cta-top left">
                              <div className="modal-cta-icon-wrap">
                                <div className="modal-cta-icon-frame">
                                  <div className="modal-cta-icon w-embed"><PhoneSvg /></div>
                                </div>
                              </div>
                              <div className="modal-cta-content">
                                <div className="modal-cta-title">Momentum Support</div>
                                <div className="uui-pricing01_top-right-price">$2,000<span className="text-span">/month</span></div>
                                <p className="modal-cta-desc">One membership, unlimited development for existing apps. Focused, fast delivery — 4 features / month</p>
                              </div>
                            </div>
                            <div className="div-block-59">
                              <div className="f-feature-card-filled">
                                <div className="f-feature-icon-wrapper-white">
                                  <img src="/images/add_14909179.webp" loading="lazy" width="40" alt="" className="image-29" />
                                </div>
                                <div className="f-margin-bottom-44">
                                  <h5 className="f-h5-heading-3">Ship new features every month</h5>
                                </div>
                              </div>
                              <div className="f-feature-card-filled">
                                <div className="f-feature-icon-wrapper-white">
                                  <img src="/images/recognition_13102873.webp" loading="lazy" width="40" alt="" />
                                </div>
                                <div className="f-margin-bottom-44">
                                  <h5 className="f-h5-heading-3">Dedicated Expert Bubble Developer</h5>
                                </div>
                              </div>
                              <div className="f-feature-card-filled">
                                <div className="f-feature-icon-wrapper-white">
                                  <img src="/images/add_3935372.webp" loading="lazy" width="40" alt="" />
                                </div>
                                <div className="f-margin-bottom-44">
                                  <h5 className="f-h5-heading-3">Design Included</h5>
                                </div>
                              </div>
                              <div className="f-feature-card-filled">
                                <div className="f-feature-icon-wrapper-white">
                                  <img src="/images/time-out_7821978.webp" loading="lazy" width="40" alt="" />
                                </div>
                                <div className="f-margin-bottom-44">
                                  <h5 className="f-h5-heading-3">Pause or Cancel Anytime</h5>
                                </div>
                              </div>
                            </div>
                          </div>
                          <a href="https://calendar.app.google/PrdZAdEpBpV5YHCKA" target="_blank" className="button-2 mmntm-btn w-inline-block">
                            <div className="button-inner mmntm-btn">
                              <div className="button-contact-avt">
                                <img loading="lazy" src="/images/contact-avt.png" alt="avatar" className="button-contact-avt-img" />
                                <img loading="lazy" src="/images/contact-avt-1.png" alt="avatar" className="button-contact-avt-img" />
                              </div>
                              <div className="button-text-wrap">
                                <div className="button-text">Reserve my spot</div>
                              </div>
                              <div className="button-icon">
                                <div className="icon-button w-embed"><ArrowButtonSvg /></div>
                              </div>
                            </div>
                            <div className="button-innershadow"></div>
                          </a>
                          <div className="dots-partern"></div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>

              {/* 3-column pricing cards */}
              <div className="w-layout-grid brix---3-columns-1-col-tablet-2">
                {/* Clarity Engine card */}
                <div className="brix---card-2">
                  <div className="brix---pricing-content-v2-2">
                    <div className="modal-cta-icon-wrap prc-icon">
                      <div className="modal-cta-icon-frame othericon">
                        <img src="/images/eye_2601288.webp" loading="lazy" width="40" alt="" />
                      </div>
                    </div>
                    <div className="brix---mg-bottom-32px-3">
                      <div className="brix---color-neutral-810">
                        <h3 className="brix---heading-h3-size-3">Clarity Engine</h3>
                      </div>
                      <div className="brix---flex-horizontal-start-2">
                        <div className="brix---mg-right-8px-2">
                          <div className="brix---color-neutral-810">
                            <div className="brix---heading-h1-size-3">$2000</div>
                          </div>
                        </div>
                      </div>
                      <div className="brix---color-neutral-811">
                        <p className="brix---paragraph-default-4">1-on-1 product discovery with a global product builder.</p>
                      </div>
                    </div>
                    <div className="brix---divider-40px-2"></div>
                    <div className="brix---mg-bottom-40px-2">
                      <div className="w-layout-grid brix---grid-1-column-gap-row-16px-2">
                        {[
                          '4 sessions with a dedicated, expert product builder',
                          'User persona definition + ICP Identification',
                          'Consistent product validation + support',
                          'Detailed step-by-step product roadmap',
                          'Low-fidelity clickable mockups included',
                        ].map((text, i) => (
                          <div key={i} className="brix---icon-list-item-wrapper-2">
                            <img alt="Check - Elements Webflow Library - BRIX Templates" src="/images/check-icon-brix-templates.svg" className="brix---icon-list-2" />
                            <div className="brix---color-neutral-810">
                              <div className="brix---text-200-list-bold-2">{text}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                    </div>
                    <a href="https://calendar.app.google/PrdZAdEpBpV5YHCKA" target="_blank" className="button-2 mmntm-btn prccard-btn w-inline-block">
                      <div className="button-inner mmntm-btn _2ndcard-btn">
                        <div className="button-contact-avt">
                          <img loading="lazy" src="/images/contact-avt.png" alt="avatar" className="button-contact-avt-img" />
                          <img loading="lazy" src="/images/contact-avt-1.png" alt="avatar" className="button-contact-avt-img" />
                        </div>
                        <div className="button-text-wrap">
                          <div className="button-text">Get Started</div>
                        </div>
                        <div className="button-icon">
                          <div className="icon-button w-embed"><ArrowButtonSvg /></div>
                        </div>
                      </div>
                      <div className="button-innershadow"></div>
                    </a>
                  </div>
                </div>

                {/* Conversion-First UX card (popular/dark) */}
                <div className="brix---card-pricing-popular-2">
                  <div className="brix---badge-wrapper-top-right"></div>
                  <div className="brix---pricing-content-v2-2">
                    <div className="modal-cta-icon-wrap prc-icon">
                      <div className="modal-cta-icon-frame">
                        <img src="/images/web_16077732.webp" loading="lazy" width="40" alt="" />
                      </div>
                    </div>
                    <div className="brix---mg-bottom-32px-3">
                      <div className="brix---color-neutral-812">
                        <h3 className="brix---heading-h3-size-3">Conversion-First UX</h3>
                      </div>
                      <div className="brix---flex-horizontal-start-2">
                        <div className="brix---mg-right-8px-2">
                          <div className="brix---color-neutral-812">
                            <div className="brix---heading-h1-size-3">$2000</div>
                          </div>
                        </div>
                      </div>
                      <div className="brix---color-neutral-812">
                        <p className="brix---paragraph-default-4">High-performance design built for real users.</p>
                      </div>
                    </div>
                    <div className="brix---divider-40px-2 mid-divider"></div>
                    <div className="brix---mg-bottom-40px-2">
                      <div className="w-layout-grid brix---grid-1-column-gap-row-16px-2">
                        {[
                          'Ship high-performance designs',
                          'Dedicated expert UI/UX designer',
                          'Design & prototype included ',
                          '2-week revision allowance',
                        ].map((text, i) => (
                          <div key={i} className="brix---icon-list-item-wrapper-2">
                            <img alt="" src="/images/check-icon-white-brix-templates.svg" className="brix---icon-list-2" />
                            <div className="brix---color-neutral-812">
                              <div className="brix---text-200-list-bold-2">{text}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                      <a href="https://calendar.app.google/PrdZAdEpBpV5YHCKA" target="_blank" className="button-2 mmntm-btn prccard-btn mid-card w-inline-block">
                        <div className="button-inner mmntm-btn">
                          <div className="button-contact-avt">
                            <img loading="lazy" src="/images/contact-avt.png" alt="avatar" className="button-contact-avt-img" />
                            <img loading="lazy" src="/images/contact-avt-1.png" alt="avatar" className="button-contact-avt-img" />
                          </div>
                          <div className="button-text-wrap">
                            <div className="button-text">Get Started</div>
                          </div>
                          <div className="button-icon">
                            <div className="icon-button w-embed"><ArrowButtonSvg /></div>
                          </div>
                        </div>
                        <div className="button-innershadow"></div>
                      </a>
                    </div>
                  </div>
                  <div className="dots-partern"></div>
                </div>

                {/* Bulletproof QA card */}
                <div id="w-node-_5b26d1ee-b4da-3aef-ae8b-03878cfdc1be-efdebd32" className="brix---card-2">
                  <div className="brix---pricing-content-v2-2">
                    <div className="modal-cta-icon-wrap prc-icon">
                      <div className="modal-cta-icon-frame othericon">
                        <img src="/images/approve_16320749.webp" loading="lazy" width="40" alt="" />
                      </div>
                    </div>
                    <div className="brix---mg-bottom-32px-3">
                      <div className="brix---color-neutral-810">
                        <h3 className="brix---heading-h3-size-3">Bulletproof QA</h3>
                      </div>
                      <div className="brix---flex-horizontal-start-2">
                        <div className="brix---mg-right-8px-2">
                          <div className="brix---color-neutral-810">
                            <div className="brix---heading-h1-size-3">$2000</div>
                          </div>
                        </div>
                      </div>
                      <div className="brix---color-neutral-811">
                        <p className="brix---paragraph-default-4">Rigorous pre-launch testing, edge-case handling</p>
                      </div>
                    </div>
                    <div className="brix---divider-40px-2"></div>
                    <div className="brix---mg-bottom-40px-2">
                      <div className="w-layout-grid brix---grid-1-column-gap-row-16px-2">
                        {[
                          '4 test cycles — triggered on demand, on your schedule',
                          'Deep edge-case detection & regression testing, anytime ',
                          'Dedicated full-stack testing expert QA engineer',
                          'Detailed bug reports, root cause analysis & stabilization',
                          'Spin up, pause, deprovision anytime — zero lock-in',
                        ].map((text, i) => (
                          <div key={i} className="brix---icon-list-item-wrapper-2">
                            <img alt="Check - Elements Webflow Library - BRIX Templates" src="/images/check-icon-brix-templates.svg" className="brix---icon-list-2" />
                            <div className="brix---color-neutral-810">
                              <div className="brix---text-200-list-bold-2">{text}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                    </div>
                    <a href="https://calendar.app.google/PrdZAdEpBpV5YHCKA" target="_blank" className="button-2 mmntm-btn prccard-btn w-inline-block">
                      <div className="button-inner mmntm-btn _3rdcard-btn">
                        <div className="button-contact-avt">
                          <img loading="lazy" src="/images/contact-avt.png" alt="avatar" className="button-contact-avt-img" />
                          <img loading="lazy" src="/images/contact-avt-1.png" alt="avatar" className="button-contact-avt-img" />
                        </div>
                        <div className="button-text-wrap">
                          <div className="button-text">Get Started</div>
                        </div>
                        <div className="button-icon">
                          <div className="icon-button w-embed"><ArrowButtonSvg /></div>
                        </div>
                      </div>
                      <div className="button-innershadow"></div>
                    </a>
                  </div>
                </div>
              </div>
            </div>
            <img
              src="/images/hero-v17-elements-brix-templates.avif"
              alt="Hero - Elements Webflow Library - BRIX Templates"
              sizes="(max-width: 2880px) 100vw, 2880px"
              srcSet="/images/hero-v17-elements-brix-templates-p-500.avif 500w, /images/hero-v17-elements-brix-templates-p-800.avif 800w, /images/hero-v17-elements-brix-templates-p-1080.avif 1080w, /images/hero-v17-elements-brix-templates.avif 2880w"
              className="brix---bg-image-full-width-2 prc-bg"
            />
          </section>

          <CtaFooter currentPage="pricing" pageId={PAGE_ID} />
        </div>
      </div>
    </>
  )
}
